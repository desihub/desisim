...
def simulate_one_healpix(ifilename,args,model,obsconditions,decam_and_wise_filters,footprint_healpix_weight,footprint_healpix_nside,seed,bal) : #I removed the value None to the bal variable    
    log = get_logger()
 ...
 ##ALMA:Added to read dla_info
  ...
  ##Luz: Since random bals have been added, I'm copying Alma's lines...
    """
    if(not args.bal or args.bal=='random'):
       trans_wave, transmission, metadata = read_lya_skewers(ifilename)
       ok = np.where(( metadata['Z'] >= args.zmin ) & (metadata['Z'] <= args.zmax ))[0]
       transmission = transmission[ok]
       metadata = metadata[:][ok]
    ...
    if args.bal:
        bal_z,bal_id = [],[] #HI column desity has been removed, since that's not argument to bal
        bal_filename=os.path.join(pixdir,"bal-{}-{}.fits".format(nside,healpix))
    """    
   ## changes made to Alma's lines up to here     
...
### Not sure about these lines -> check with Jaime 
...
### Luz: adding later lines, adapted to bal case
    """
    elif(args.bal=='random'):
        log.info('Adding BALs randomly')
        min_trans_wave=np.min(trans_wave/1215.67 - 1) # which wavelength should I use this?
        for ii in range(len(metadata)):
            if min_trans_wave < metadata[ii]['Z']: 
               idd=metadata['MOCKID'][ii]
               balss, bal_model = insert_bals(trans_wave, metadata[ii]['Z']) # if lines 210-233 are included for BALs, then bal_model should be modified accordingly 
               if len(balss)>0:
                  transmission[ii]=bal_model*transmission[ii]
                  bal_z+=[ibal['z'] for ibal in balss]
                  bal_id+=[idd]*len(balss)    
...
## Adding BALs according to previous comment here
    if (args.balprob):
