...
#- TODO: This doesn't really belong in obs module
def _write_simpix(img, camera, flavor, night, expid):
    """
...
def new_arcexp(nspec=None, nspectrographs=10):
    ...
    w = d['AIRWAVE']
    phot = np.tile(d['ELECTRONS'], nspec).reshape(nspec, len(w))
    ...
    #- Update obslog that we succeeded with this exposure
    update_obslog('arc', expid, dateobs)

def new_flatexp(nspec=None, nspectrographs=10):
    expid = get_next_expid()
    dateobs = time.gmtime()
    night = get_night(utc=dateobs)
    simdir = io.simdir(night, mkdir=True)
    ...
    #- Update obslog that we succeeded with this exposure
    update_obslog('flat', expid, dateobs)
