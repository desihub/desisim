...
def load_s2n_values(objtype, nights, channel, exposures=None):
    fdict = dict(waves=[], s2n=[], fluxes=[])
    for night in nights:
        if exposures is None:
            exposures = desispec.io.get_exposures(night, raw=True)
        for exposure in exposures:
            fibermap_path = desispec.io.findfile(filetype='fibermap', night=night, expid=exposure)
            ...
            for ii in range(10):
                ...
                cframe_path = desispec.io.findfile(filetype='cframe', night=night, expid=exposure, camera=camera)
                try:
                    cframe = desispec.io.read_frame(cframe_path)
                except:
...
if __name__ == '__main__':
    ...
    s2n_values = load_s2n_values('ELG', nights, 'b', exposures=exposures)
