...
### from desitarget.mocks.io import read_mock_spectra
### from fiberassign.io import read_fiberassign
...
reference_conditions['DARK']['AIRMASS'] = 1.3
...
reference_conditions['GRAY']['AIRMASS'] = 1.3
...
def newexp(fiberassign, expid, night, mockdir, fiberassigndir=None,
    obsconditions=None, outsimdir=None, nspec=None):
    '''
    ...
    Args:
        ...
        night: (int or str) YEARMMDD
        mockdir: location of mock input truth files
    ...
    if obsconditions is None:
        ...
        #- filename
            ...
            if obsconditions.endswith('.fits'):
                allobs = astropy.table.Table.read(obsconditions, format='fits')
            elif obsconditions.endswith('.fits'):
                allobs = astropy.table.Table.read(obsconditions, format='ascii.ecsv')
            else:
                #- unknown format, hope for the best
    ...
    if outsimdir is not None:
        simfile = desisim.io.write_simspec(sim, fibermeta, expid, night,
            outdir=outsimdir, header=header)
        fmfile = desispec.io.findfile('fibermap', night, expid, outdir=outsimdir)
        fm = fibermeta2fibermap(fibermeta)
        fm.meta['NIGHT'] = night
        fm.meta['EXPID'] = expid
        fm.write(fmfile, overwrite=True)
...
def simulate_spectra(wave, flux, meta=None, obsconditions=None, galsim=False):
    ...
    #- unitless versions for convenience
...
def get_source_types(fibermap):
    ...
    match that, though specsim doesn't handle those types.
